...
def _write_external_plugin(user_root: Path, dependency_root: Path) -> Path:
    ...
    (plugin_root / "plugin.py").write_text(
        ...
        "    import pathlib, platform, regex, sys, sysconfig\n"
        "    import regex._regex as binary\n"
        "    expected = pathlib.Path(expected_runtime_root).resolve()\n"
        ...
        "    if expected not in executable.parents:\n"
        "        raise RuntimeError(f'Python did not execute from bundled runtime: {executable}')\n"
        "    if expected not in origin.parents or origin.suffix != '.so':\n"
        "        raise RuntimeError(f'regex binary did not import from bundled runtime: {origin}')\n"
        "    match = regex.fullmatch(r'\\d+', '313')\n"
        "    if match is None:\n"
        "        raise RuntimeError('regex binary import did not execute')\n"
        "    return {'python_executable': str(executable), 'binary_import_path': str(origin),\n"
        "            'platform': sysconfig.get_platform(), 'binary_match': match.group()}\n"
        "\n"
        "def descriptor():\n"
