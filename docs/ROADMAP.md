...
- 普通透明内联元素中的相邻 XHTML `text`/`tail` 槽合并为一个 Segment；未知
  结构和 `br` 形成边界，Ruby 继续保持独立语义 Segment。
- EPUB Adapter 在不透明状态中保存 Ruby 原始结构和复合定位；不引入宿主
  通用 DOM、排版树或跨格式中间表示。
- 普通复合 Segment 保存有序槽定位；纯译文写入首槽并清空其余槽，双语导出在
  末槽后追加目标文本，同时保留原内联标签骨架。
...
- 增加请求延迟、HTTP 错误、重试/限流等待次数、usage 覆盖状态，以及日志
  级别、项目、阶段过滤、搜索和暂停自动滚动。
...
进入条件：普通内联合并已在真实 EPUB 中稳定验证，且不引入通用 DOM、通用
Adapter 校验 Hook 或自由 HTML 输出协议。

- 增加默认关闭、EPUB 专用的导入模式，为模型提供防碰撞、可验证的格式范围标记；
  标记不携带 attrs，原 attrs 保留在 Adapter 不透明状态。
- 翻译、校对和润色结果在完成前都必须验证标记完整、顺序正确且可嵌套；失败时
  明确拒绝，不静默退回普通模式。
- 导出按经过验证的标记恢复原始节点和 attrs；不接受模型任意生成 HTML，不按
  原文字数启发式拆分目标，也不把标记协议提升为宿主通用接口。
