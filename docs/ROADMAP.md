...
Provider 判断、静默 fallback、任意格式互转或未经真实需求
...
- CLI 与 Web 文件管理均已实现。
- Web 项目概览支持显式确认后删除整个自包含项目目录；存在未完成 Run 时拒绝，
  删除后不保留项目数据或最近外部项目路径。
...
- Web 新建和打开项目支持由服务端提供的单层目录浏览；只列目录，不递归扫描或
  返回文件内容，`...` 返回上级目录。Windows 驱动器根目录额外列出逻辑驱动器；
  未装载或不可用的驱动器保留显示但不可进入，可刷新后重新探测介质状态。
...
- 普通日志不得包含 API Key、鉴权 Header、未脱敏请求/响应 Payload 或源文
  调试数据。
- 阶段状态同时显示已完成、失败、待处理和总 Segment；失败数量可跳转到错误
  Segment，错误行只显示安全错误分类和摘要，后续成功会清除该 Segment 的错误状态。
- 术语扫描严格拒绝不完全等于 `{"type":"end"}` 的 end 行，但先保存已经解析的
  合法候选。活动扫描可临时导出 JSON/CSV；在扫描未运行且明确确认后，可按既有
  合并、冲突和 override 规则部分发布为 SQLite 中的普通术语库，立即供下游阶段使用。
  部分发布只结束活动扫描状态，保留历史 scans、candidates 和 Run；下一次扫描
  创建新的活动任务。普通日志与错误摘要继续遵守脱敏约束。
- 术语页区分“移除”和“彻底删除”：移除保留 disabled override 以继续忽略后续扫描；
  彻底删除同时清除术语记录和该 override，保留历史扫描记录并允许后续重新发现。
...
- File、Segment、结果和 Adapter 状态在同一事务内保持一致；一个 EPUB 仍是
  一个 File。
- 数据库包含明确 schema version，未知版本快速失败；新项目不创建 JSONL 运行路径，
  旧 JSONL 项目只提示重新创建。可读 Run manifest 及其快照仍保存在 Run 目录。

## Stage 19：20,000+ Segment Web 性能（已完成）
...
- `orjson` 只有在窗口化完成后仍由剖析确认序列化为主要瓶颈时才允许引入。
  ...
  瓶颈时才允许引入。
- 本阶段未引入 `orjson` 或 `lxml`；二进制依赖继续留到后续三平台 sidecar
  打包验证后再由真实剖析决定。

## Stage 20：中文与英文国际化（已完成）
...
CLI 与桌面模式统一监听 `0.0.0.0`，由中间件按请求守卫非回环访问：未启用共享
时拒绝全部非回环客户端；启用后始终放行本机回环，并按所选接口网段放行非回环
客户端（`0.0.0.0` 表示全部网段）。`lan.bind_address` 是接口选择语义，保存后
即时生效，无需重启。
...
- 构建并验证 macOS Tauri 应用、Python sidecar、系统钥匙串、用户数据目录和
  原生选择器。
- 提供未签名 ad-hoc 安装包（开源兴趣项目，不签名不公证）及中英文首次启动
  引导。
...
## Stage 23.1：导出文件浏览与局域网下载（已完成）

- 导出保持写入项目内 `output/`，不增加主机另存位置（下载能力已覆盖该需求，避免破坏项目自包含边界）。
- 新增常驻已导出文件浏览：`GET .../exports` 递归列出 `output/` 全部文件（始终排除 `.staging`），返回相对路径、大小、mtime；导出成功后在页面刷新并高亮新文件。
- 下载与删除统一限定在项目 `output/` 内：所有 `file` 参数必须是纯相对路径，resolve 后不得逃逸 output 根且必须是普通文件；路径穿越、绝对路径、符号链接整体拒绝，局域网客户端无法借道读取主机任意文件。
- 逐文件下载返回 attachment；勾选文件可打包为 zip（stdlib `zipfile`，保留相对路径），不做持久化下载清单。
- 删除仅限文件、不清理目录；局域网设备与主机同权限（复用现有登录/网段守卫，不新增分支）。
- Tauri 桌面下载每次弹系统原生保存对话框：`save_export` 命令经极简 HTTP GET 从回环 sidecar 拉取字节后写盘（零新依赖）；浏览器（主机或 LAN）保持标准 attachment 下载。

## Stage 23.2：导出页标签化与列表可用性（已完成）

- 导出页拆为两个标签：`导出`（生成表单 + 源文件选择）与 `浏览输出目录`（已导出文件管理）；生成成功提示提供前往浏览标签的入口，新导出文件在浏览标签中高亮。
- 已导出文件列表改为独占整页高度滚动，并按路径客户端过滤；源文件选择列表新增按文件名过滤，概览文件列表限高滚动（不引入虚拟化依赖，覆盖数百至数千文件场景）。
- 浏览器模式下文件下载控件统一为内联 flex 垂直居中，修正 `<a>` 形态按钮文字贴顶问题。
- `GET .../exports` 列表忽略 `.DS_Store`（Finder 遗留文件），下载与删除语义不变。

## Stage 23.3：导出页标签栏稳定与双栏工作台（已完成）

- 导出页改为恒定的 flex 列布局（取消网格自动行拉伸），标题与标签栏在切换标签时位置、高度完全不变。
- 导出标签重构为双栏工作台：左栏为生成表单（阶段/格式/双语/生成），右栏为过滤 + 文件选择列表，列表占满整页剩余高度（取消 280px 上限）；页面加宽至与概览一致。
- 项目概览文件列表限高从 `40vh` 提升到 `min(55vh, 560px)`。
