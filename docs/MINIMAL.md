...
吞吐量只使用完整端点 usage 除以当前运行耗时；usage 缺失或不完整时，Token 计数和吞吐量均显示不可用。右侧请求/ 响应列表只轮询轻量摘要；双击请求或选择“查看”后，按需显示请求、Content、Reasoning 和尝试详情，运行中的已打开详情随仪表盘刷新。活跃请求详情不计入已结束详情的 200 条上限。
