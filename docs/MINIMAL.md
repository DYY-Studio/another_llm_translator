...
内容概括是术语页中的实验性子页面，服务于需要快速了解各个文件内容边界的读者，
也供译者在阅读术语和片段时回看上下文。它不声称替代人工阅读，也不把概括自动
加入翻译、校对或润色的 `reference_context`。

### 边界、选择与入口

概括的最小单位固定为 `(file_id, part_id)`。这是 Document Adapter 提供的通用
内容边界；TXT、EPUB 或外部 Adapter 都只通过 File 和 part 提供内容，不为某一种
格式写特殊的章节识别或导出分支。空 Segment 不提交模型，也不计入边界覆盖数量。

术语页的“更多”菜单中提供“内容概括 · 实验”和术语自动决策入口。只有从概括子页
显式启动的“扫描术语并补概括”才读取已保存的参与选择；普通术语命令、CLI 和
`run-all` 仍只执行原有术语阶段。概括参与选择只决定哪些边界生成片段概括，永远
不缩小术语扫描范围；开启联合模式时必须关闭术语阶段的
`chunking.cross_boundary_batching`，冲突会直接提示用户处理。

参与选择器支持项目级全选/取消全选、当前筛选结果全选/取消全选、按 File 的半选
状态和 `(file_id, part_id)` 选择。搜索只改变显示范围，隐藏的已选项仍保留，并显示
已选总数。聚合和 Markdown 导出各自使用独立的选择器，不复用参与选择，也不会因为
取消选择删除已有概括。

### 请求模式与持久化

联合请求复用术语扫描已经发送给模型的源文。术语阶段的片段概括响应是严格 JSONL：
一条或多条 `summary`、联合模式下其后的零或多条 `term`，最后一条
`{"type":"end"}`。单条 `summary` 可以省略 `refs`，宿主会按本次请求的全部
`source_refs` 自动补齐；多条时每条都必须显式提供 `refs`，各条引用不能重叠且合并后
必须恰好覆盖全部输入，术语仍只能来自源文。系统支持三种模式：

- `terms-only`：只提取术语；
- `terms+fragment-summary`：同时提取术语和当前边界的一个或多个片段概括；
- `summary-only`：术语已完成时只补缺少的一个或多个片段概括。

每个片段概括 `summary` 分别保存为一个 fragment artifact；其 `source_range` 只保留
该 summary 的输入子集，持久化 `refs` 在该 artifact 内从 `1` 开始重编号。`content_summary`
聚合阶段仍要求每个边界只生成一条 summary，以保持每个 `(file_id, part_id)` 只有一个
完整概括结果。若一次已完成的 fragment 覆盖当前边界的全部非空 Segment，系统会在生成
时保留该 fragment，并复制发布一个 `kind = full`、来源为 `adopted_fragment` 的完整结果，
不再次调用模型；多个 fragment 不会自动合并。

已有成功的概括只按已落盘的源文摘要、模型文本摘要、Prompt 摘要和模型等 provenance
校验后复用；这里不构成完整阶段设置指纹的复用承诺。部分响应只重试失败的结果类型；
...
概括时，宿主会在计算覆盖范围和发起请求前，物理删除本次参与选择中各边界的全部
`fragment` 记录；未选边界、`full`/`reduction` 结果和 Run 审计记录不受影响。重做失败
不会恢复被删除的旧片段，失败状态由当前任务明确显示；dry-run 不删除数据库记录。
Prompt、Preset、Adapter 或其他设置不匹配时，任务层会明确报错并要求用户决定；当前
概括子页没有专门的复用/重做控件。`summary-only` 回填不创建新的术语任务、不改候选、
不重新发布术语库。

片段概括和聚合结果保存在 SQLite 的内容概括记录中。每条记录保留 `(file_id,
part_id)`、源文/模型输入摘要、Prompt/模型信息、原始 Segment 范围及引用。内部
...
记录标记为源已变化，不能当作完整当前概括导出。用户仍可查看历史记录，并决定是否
重新生成。Prompt、Preset 或其他设置不匹配时由任务层明确报错并要求用户决定；当前
概括子页没有专门的复用/重做控件，也不会在非 force 启动时静默清空结果。

### 手动聚合、阅读与导出

单个 fragment 已覆盖整个当前边界时，生成阶段已经发布可读取、可统计和可导出的完整结果。
历史上只有 fragment 的记录不会自动迁移；多个 fragment 或需要重新组织措辞时，用户仍可
在概括子页手动选择边界并执行“聚合完整概括”。聚合预检
...
在当前应用会话中按项目恢复；返回术语库才显式离开子页，切换到其他阶段后返回仍
停留在原子页。

Markdown 导出按用户单独选择的边界生成完整概括，并附带 File/part、持久 Segment
标识和原文引用。未选择、缺少完整概括、源已变化或路径无效时直接报错；已经存在
的历史完整概括可以在导出选择中重新选用。质量判断使用代表性材料的人工检查清单
和示例输出，不设随机百分比门槛。

人工检查清单固定检查以下项目：每个被选边界是否完整覆盖当前非空 Segment；概括是否
只陈述可由引用原文支持的内容；`refs` 是否覆盖且只覆盖本次输入；跨 File/part 是否
被错误合并；模型失败、取消、源文变化后是否保留已成功结果并能继续；Markdown 是否
包含边界、持久 Segment 标识和可读原文。示例输出只用于复现检查步骤，不作为质量分数：
...
Alice 进入房间，Bob 随后向她挥手。

### 引用与原文

- `F0001-S000001`：Alice entered.
- `F0001-S000002`：Bob waved.
```
...
- HTTPX 的远端协议连接中断沿现有退避消耗一轮重试；响应内容解码失败直接失败。
...
本地 Web 将相同安全摘要写入应用级 `logs/app.log`，按大小轮转，不因项目切换而清空；同时在有界内存中保留当前进程的结构化日志供仪表盘读取。仪表盘为当前进程的每个活跃 Web 任务独立保留逻辑 LLM 请求；HTTP 重试只追加到同一请求。请求摘要只存在当前进程内；同一项目开始新 Run 时清除该项目上一 Run 的请求，所有活跃请求详情持续可读，已结束请求最多保留最近 200 条，超出部分从请求列表和内存中移除。
...
吞吐量只使用完整端点 usage 除以当前运行耗时；partial usage 保留已观测 Token，但吞吐量显示不可用；没有任何可观测 usage 时，Token 与吞吐量均显示 unavailable。右侧请求/ 响应列表只轮询轻量摘要；双击请求或选择“查看”后，按需显示请求、Content、Reasoning 和尝试详情，运行中的已打开详情随仪表盘刷新。同一项目开始新 Run 时清除上一 Run 的请求；已结束请求最多保留最近 200 条，超出部分从列表和内存中移除，活跃请求不计入该上限。
