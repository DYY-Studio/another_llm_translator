...
参与选择器在没有搜索词时显示“全选/取消全选”，有搜索词时显示“筛选结果全选/筛选
结果取消全选”；后者只作用于当前可见边界。按 File 的半选状态和 `(file_id, part_id)`
选择保持不变。搜索只改变显示范围，隐藏的已选项仍保留，并显示已选总数。片段生成和
完整聚合都复用已保存的参与选择，并通过通用运行弹窗确认；只有 Markdown 导出使用独立
选择器，不会因为取消参与选择删除已有概括。
...
不再次调用模型；多个 fragment 不会自动合并。完整结果、递归压缩结果和自动采用的
完整结果都以同一格式保存直接依赖的 `record_id`、`kind`、文本摘要和源文摘要；
`input_digest` 是按依赖顺序计算的规范摘要，不复制 fragment 的输入摘要。
...
子摘要或分区发生变化时，完整概括标记为过期。状态为 `stale`、源文变化、依赖变化和
无法验证 provenance 分别记录 `stale_status`、`source_changed`、`dependency_changed`
...
标识和原文引用。未选择、缺少可用文本的完整概括或路径无效时直接报错；已经存在的
过期完整概括仍可在导出选择中重新选用并导出，页面会提示建议重新生成。质量判断使用
代表性材料的人工检查清单
和示例输出，不设随机百分比门槛。
