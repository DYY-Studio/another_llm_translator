...
联合请求复用术语扫描已经发送给模型的源文。响应是严格 JSONL，顺序为一条
`summary`、零或多条 `term`、最后一条 `{"type":"end"}`；summary 使用请求内
短引用 `refs` 指向本次 `source_segments`，术语仍只能来自源文。系统支持三种模式：

- `terms-only`：只提取术语；
- `terms+fragment-summary`：同时提取术语和当前边界的片段概括；
- `summary-only`：术语已完成时只补缺少的片段概括。
...
之后再次启动只处理缺失或失败范围。Prompt、Preset、Adapter 或其他设置不匹配时，
任务层会明确报错并要求用户决定；当前概括子页没有专门的复用/重做控件。`summary-only`
回填不创建新的术语任务、不改候选、不重新发布术语库。
...
概括子页没有专门的复用/重做控件，也不会静默清空结果。

### 手动聚合、阅读与导出

片段概括生成后必须由用户在概括子页手动选择边界并执行“聚合完整概括”。聚合预检
