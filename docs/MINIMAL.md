...
而清空；同时在有界内存中保留当前进程的结构化日志供仪表盘读取。LLM 响应中的
Reasoning 只保存在当前运行的有界内存日志中，不经过普通 logger、不写入磁盘，
下一次运行会替换，应用重启后丢失。
...
HTTP 错误、重试、限流等待、累计输入/输出 Tokens 与总吞吐量。日志支持级别、
项目、阶段和文本过滤，并可暂停自动滚动。吞吐量只使用完整端点 usage 除以当前
运行耗时；usage 缺失或不完整时，Token 计数和吞吐量均显示不可用。仪表盘可查看
本次运行的内存 Reasoning，但日志和指标不记录鉴权 Header、请求/响应 Payload、
Prompt 或源文。
...
- Web 仪表盘在项目切换和窄屏下可见全局日志、当前请求指标及精确 usage；过滤、
  搜索和自动滚动暂停生效，usage 不完整时不显示估算吞吐量，Reasoning 不落盘。
