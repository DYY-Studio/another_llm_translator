...
阶段和原因；请按提示修复或移除对应插件后重启应用。插件发现与加载规则见
[Adapter 契约](ADAPTERS.md#3-可信-python-插件宿主beta)。
...
桌面应用要求 macOS 13 或更高版本，并自带 Python 运行时；使用桌面应用无需另行安装 Python。

Windows 和 Linux 桌面版本当前尚未公开提供。其他平台可以使用本地 Web。

如果桌面窗口显示后端启动失败，页面会列出失败阶段、启动命令和原因。插件及其依赖无法加载时，
应用启动会失败并显示插件路径、阶段和原因；请按提示修复或移除对应的用户插件后重启。插件发现与加载规则见
[Adapter 契约](ADAPTERS.md#3-可信-python-插件宿主beta)。
...
Preset 可以显式关闭连续读取超时，但不会取消连接超时。流式请求的协议和失败处理由 Adapter 定义，具体规则见
...
项目有运行中任务时，仍可以切换项目、刷新页面并查看概览、术语、阶段结果和任务进度。运行中只能查看；文件变更、其他修改和项目修复需等待任务结束或取消后再操作。
...
术语自动终审默认关闭；开启后会为待审术语提供更广上下文并明确裁决，没有待审术语时不会
发起终审请求。终审的额外用量不计入上述估算，结果仍需人工应用；终审失败会明确停止。
...
在术语页打开“更多”→“内容概括 · 实验”。页面按文档提供的内容边界显示。

左侧勾选“参与概括”的范围后，点击“扫描术语并补概括”。该选择会保存在当前项目中。
若当前设置无法保证概括边界，启动会明确失败并要求先调整设置。

没有搜索词时可以全选或取消全选；搜索后，相同操作只影响当前筛选结果。筛选不会清除
隐藏的选择或删除已有结果。片段生成和完整聚合使用当前参与范围，Markdown 导出则有
独立选择，因此可以导出另一组已有完整概括。

片段结果显示在“片段概括”页签；多个片段会按原文顺序聚合。点击“查看来源”可以检查概括引用的原文。

取消、网络或格式错误时，已经成功的结果仍可查看，再次启动会补齐缺少或失败的范围。源文、
Prompt、模型或依赖结果发生变化时，页面会显示“过期”或“无法验证”，请重新生成。旧完整概括
仍可查看和导出；版本保留和结果复用规则见[最小产品规范](MINIMAL.md#52-内容概括实验)。
...
在项目概览点击“连续运行”打开专用对话框。选择术语、翻译或校对作为起点，再按顺序勾选
相邻阶段；取消其中一步会同时取消后面的阶段。术语到翻译必须包含术语自动决策并在翻译前完成。校对
起点只有在项目已有完整翻译时可用；翻译起点不要求已有术语库。

对话框会列出每一步的 Preset、数量、运行中的 Run 和预检阻塞。已有结果可以选择保留待处理
项、复用不同设置的已完成结果或强制重做；复用和强制重做不能同时选择。每个未完成 Run 都必须
明确选择“续用”或“结束并新建”。结束术语自动决策 Run 还必须选择强制重做。

术语自动决策位于中间步骤时自动终审固定开启，启动前必须勾选授权应用术语决策；位于末尾时
可以选择是否自动终审，但草案仍需人工应用。选择校对后继续润色时，校对成功结果会自动应用，
人工拒绝的条目保留原文，润色建议仍需人工应用。任何步骤失败或取消都会停止后续步骤，已完成
步骤的结果和 Run 仍然保留。

任务面板会显示“连续运行”、当前阶段和每一步状态。不同阶段的数量不合并显示为 Segment 总进度；
预检阻塞、模型错误、结果指纹不一致和恢复选择缺失都会直接显示原因。
...
在“设置”中切换到“全局 → 存储”，即可查看应用管理数据和已登记项目的占用汇总。页面打开或
...
CLI 与 Web 共用项目存储、阶段执行、限速和恢复行为。以下示例在源码目录执行；
