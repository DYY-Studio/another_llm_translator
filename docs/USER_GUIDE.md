...
才会生效。用户数据目录中的“提示词仓库”按阶段和语言保存命名模板。载入仓库条目同样只
更新草稿，显式保存项目 Prompt 后才会参与运行。
...
Web 会记住上次选中的项目 ID。外部项目先按已保存路径重新打开，再关联活动任务。顶部状态条
只展示当前项目；其他运行中的项目只在项目选择器显示“运行中”提示。切换到相应项目后，
才能查看详情或取消任务。应用不提供跨项目任务中心、并排进度、批量取消或统一操作。
...
术语导入会先完整校验再合并，不会删除文件中未出现的条目。人工 override 优先于自动扫描
结果，冲突不会被静默裁决。
