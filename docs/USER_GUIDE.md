...
处理设置冲突。Prompt、Preset 或 Adapter 不匹配时任务层会明确报错并要求用户决定，
当前概括子页没有专门的复用/重做控件。

选择器提供项目级“全选/取消全选”、当前筛选结果全选/取消全选、按 File 的半选状态和
单个边界选择。筛选只影响显示，不会隐藏已选数量，也不会删除结果。聚合与导出分别在
自己的选择对话框中选择边界，与参与选择相互独立。
...
和原文 Segment 引用。缺少当前有效的完整概括或路径无效时导出失败。取消、网络失败和
格式失败会保留已成功结果并显示状态，后续再次启动会继续缺少或失败部分；源文件变化后
旧结果保留为历史，但需要重新生成才能作为当前结果使用。
