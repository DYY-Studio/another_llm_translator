...
生成的片段概括显示在“片段概括”页签。点击“聚合完整概括”前，系统会检查所选边界的
片段是否完整覆盖当前源文；缺失、重叠或源文变化会直接报告。单个片段已经覆盖整个边界
时会直接采用，不额外调用模型；多个片段会按原文顺序聚合，过长时分层压缩并保留来源
引用。结果有“查看来源”入口，可按需打开引用的原文 Segment。
