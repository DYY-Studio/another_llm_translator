...
参见[开发指南](DEVELOPMENT.md#5-macos-桌面开发与打包)。

Windows 和 Linux 桌面版本当前尚未公开提供。其他平台可以使用本地 Web。

## 2. 配置模型连接与凭据

首次翻译前，至少需要一个可用的 LLM Preset。Preset 保存端点、模型、限流参数、Adapter
...
EOF”的 clean EOF；其他 Adapter 仍要求其声明的终止事件。
...
每次移动都会立即保存，失败时恢复移动前顺序。重排不会改变 File/Segment ID，也不会删除
已有译文、历史结果或 Adapter 状态。
...
关联术语可能需要串行决策，无法始终用满并发。新增冲突也会增加请求，因此实际用量可能
明显高于估算。为解决冲突，同一组术语必须放入一个 Chunk；该 Chunk 的输入 Token 可能
超过项目限制。
...
在术语页打开“更多”→“内容概括 · 实验”。概括按通用 `(file_id, part_id)` 内容边界
显示，适用于内置或外部 Document Adapter；页面不会按 TXT、EPUB 等格式猜测章节。

左侧勾选“参与概括”的边界后，点击“扫描术语并补概括”。参与选择会保存到当前项目，
但只影响概括生成，术语扫描仍覆盖整个项目。项目内已有术语而缺少概括时，系统只补
`summary-only`；已有结果会在源文/模型文本/Prompt 摘要和模型等已记录 provenance
通过校验后复用。标准术语按钮、CLI 和 `run-all` 不会因为这个
选择自动生成概括。若术语阶段启用了跨 File/part 合并，联合模式会明确拒绝，需由用户
处理设置冲突。启动前会打开通用运行弹窗，展示当前 Preset、已有结果和未完成 Run，
并要求用户明确选择续用、复用或重做；完整聚合不支持续用旧 Run，会明确要求结束旧
Run 后重新启动。

选择器在没有搜索词时显示“全选/取消全选”，有搜索词时显示“筛选结果全选/筛选结果取消
全选”；筛选模式只作用于当前可见边界。筛选只影响显示，不会清除隐藏的已选项，也不会
删除结果。参与选择同时作为片段生成和完整聚合的范围；两者都使用通用运行弹窗。Markdown
导出仍在独立选择对话框中选择边界，因此可以导出不同于当前参与范围的已有完整概括。

生成的片段概括显示在“片段概括”页签。单个片段已经覆盖整个当前边界时，系统会在生成时
保留 fragment 并自动发布一个完整结果，不额外调用模型；多个片段会按原文顺序聚合，过长
时分层压缩并保留来源引用。历史上只有 fragment 的记录不会自动迁移，需要时仍可点击
“聚合完整概括”；结果有“查看来源”入口，可按需打开引用的原文 Segment。
...
同一项目的写任务互斥；已有运行中的任务时，第二个写任务会明确失败。WebTask 的状态仅存在于
当前 Web 进程。页面刷新或关闭后重新打开时，前端会通过活动任务接口重新发现仍在排队、运行
或取消中的任务，并恢复当前项目的进度、Token、错误和取消入口。

完成、失败或已取消的任务不会在重新打开时重建。项目结果和 Run 记录仍会持久化，但 Web
进程重启不会恢复旧的取消句柄。

Web 会记住上次选中的项目 ID。外部项目先按已保存路径重新打开，再关联活动任务。顶部保留
当前项目的详细任务状态条，并提供紧凑的“任务 N”全局面板，列出当前进程所有排队、运行或
取消中的任务，显示项目、阶段、状态、进度与 Token 可用性；可以打开对应项目并逐项取消任务。
如果任务对应的项目尚未在当前项目缓存中，面板会先刷新项目列表再打开；项目仍不可用时显示
明确错误。应用不提供任务历史、批量取消、持久化队列或跨进程调度。
...
开发或特殊部署可以用 `ANOTHER_LLM_USER_ROOT` 指向其他用户数据根目录。项目数据库是
项目元数据、File、Segment、术语、阶段结果和 Run 索引的权威存储。

### 日志与敏感内容

普通日志不会保存完整 Prompt、源文、鉴权 Header 或未脱敏请求正文。Debug 模式会额外保存
完整请求、响应、Attempt 和 Chunk 信息，其中可能包含 Prompt 与源文；处理敏感材料时不要
启用 Debug。
